...
class CustomerController extends Controller
{
    ...
    public function update(UpdateCustomerRequest $request,Customer $customer){
        ...
        if ($request->hasFile('image')) {
            ...
            $customer->image = PhotoUpload::storeImage($customer, $file);
        }
        $customer->update();
    ...
    public function  destroy(Customer $customer)
    {
        ...
        $customer->delete();
        FcmTokenKey::where('customer_id',$customer->id)->delete();
        return 'success';
    }
    ...
    public function serverSide()
    {
        ...
        ->addColumn('image', function ($each) {
            return '<img src="' . $each->photo . '" class="thumbnail_img"/>';
        })
        ...
        ->rawColumns(['image','action','is_banned'])
        ->toJson();
    }
}
