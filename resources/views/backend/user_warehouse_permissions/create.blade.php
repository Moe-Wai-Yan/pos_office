...
@section('content')
<div class="container">
    <h2 class="mb-3">Add User Warehouse Permission</h2>

    <form action="{{ route('user-warehouse-permissions.store') }}" method="POST">
        @csrf
    ...
    <select name="warehouse_id" class="form-select" required>
        ...
        @foreach($warehouses as $warehouse)
            <option value="{{ $warehouse->id }}"
                {{ old('warehouse_id', $permission->warehouse_id ?? '') == $warehouse->id ? 'selected' : '' }}>
                {{ $warehouse->name }}
            </option>
        @endforeach
    </select>
    ...
    <div class="col-md-6">
        <div class="form-check mb-2">
            <input type="checkbox" class="form-check-input" name="can_view_reports" value="1"
                {{ old('can_view_reports', $permission->can_view_reports ?? false) ? 'checked' : '' }}>
        ...
        <div class="form-check mb-2">
            <input type="checkbox" class="form-check-input" name="can_custom_price" value="1"
                {{ old('can_custom_price', $permission->can_custom_price ?? false) ? 'checked' : '' }}>
